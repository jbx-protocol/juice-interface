const { ethers } = require("hardhat");
const { use, expect } = require("chai");
const { solidity } = require("ethereum-waffle");

use(solidity);

const projectId1 = 0;
const projectId2 = 1;
const permissionIndexes1 = [0, 7, 77];
const permissionIndexes2 = [1, 7, 58];
const unusedPermissions = [2, 4, 5, 23, 59, 88, 92, 224, 249, 255];

let contract;
let caller;
let operator1;
let operator2;

const contractName = "OperatorStore";

module.exports = () => {
  before(async () => {
    [caller, operator1, operator2] = await ethers.getSigners();
    const contractArtifacts = await ethers.getContractFactory(contractName);
    contract = await contractArtifacts.deploy();
    await contract.deployTransaction.wait();
    // Add operators.
    const tx = await contract
      .connect(caller)
      .addPermissionsToOperators(
        [projectId1, projectId2],
        [operator1.address, operator2.address],
        [[...permissionIndexes1, ...permissionIndexes2], permissionIndexes1]
      );
    await tx.wait();
  });
  describe("Success cases", () => {
    describe("removePermissionsFromOperators(...)", () => {
      it("Should remove permissions correctly", async () => {
        const tx = await contract
          .connect(caller)
          .removePermissionsFromOperators(
            caller.address,
            [projectId1, projectId2],
            [operator1.address, operator2.address],
            [permissionIndexes2, permissionIndexes1]
          );
        const receipt = await tx.wait();

        expect(receipt.confirmations).to.equal(1);

        // Verify events
        expect(receipt.events).to.have.lengthOf(3);
        expect(receipt.events[0])
          .to.have.property("event")
          .that.equals("SetPackedPermissions");
        expect(receipt.events[1])
          .to.have.property("event")
          .that.equals("SetPackedPermissions");
        expect(receipt.events[2])
          .to.have.property("event")
          .that.equals("RemovePermissionsFromOperators");
      });
      it("Should work if called by the operator being removed", async () => {
        const tx = await contract
          .connect(operator1)
          .removePermissionsFromOperators(
            caller.address,
            [projectId1],
            [operator1.address],
            [permissionIndexes2]
          );
        const receipt = await tx.wait();
        expect(receipt.confirmations).to.equal(1);
      });
    });
    describe("permissions(...)", () => {
      it("Should have updated/stored the correct packed permissions.", async () => {
        // Get the stored packed permissions value for operator1.
        const storedPackedPermissions1 = await contract.permissions(
          caller.address,
          projectId1,
          operator1.address
        );
        // Get the stored packed permissions value for operator2.
        const storedPackedPermissions2 = await contract.permissions(
          caller.address,
          projectId2,
          operator2.address
        );
        // Calculate packed value.
        const expectedPackedPermissions1 = permissionIndexes1
          // get values not in permissionIndexes2 only.
          .filter(value => !permissionIndexes2.includes(value))
          .reduce(
            (sum, i) => sum.add(ethers.BigNumber.from(2).pow(i)),
            ethers.BigNumber.from(0)
          );

        // Expect the packed value to be correct.
        expect(storedPackedPermissions1).to.equal(expectedPackedPermissions1);
        // Expect the packed values to be 0.
        expect(storedPackedPermissions1).to.equal(expectedPackedPermissions1);
        expect(storedPackedPermissions2).to.equal(0);
      });
    });
    describe("hasPermission(...) & hasPermissions(...)", () => {
      it("Should report as having no permissions.", async () => {
        const uniquePermissionIndexes1 = permissionIndexes1.filter(
          value => !permissionIndexes2.includes(value)
        );
        // The operator should only have the permissions specified.
        await Promise.all(
          [
            ...permissionIndexes1,
            ...permissionIndexes2,
            ...unusedPermissions
          ].map(async i => {
            // Get the stored packed permissions value.
            const storedHasPermission1 = await contract.hasPermission(
              caller.address,
              projectId1,
              operator1.address,
              i
            );
            const storedHasPermission2 = await contract.hasPermission(
              caller.address,
              projectId2,
              operator2.address,
              i
            );
            // Expect the permission state to match.
            expect(storedHasPermission1).to.be.equal(
              // get values not in permissionIndexes1 only.
              uniquePermissionIndexes1.includes(i)
            );
            expect(storedHasPermission2).to.equal(false);
            // Make sure that checking for multiple permissions works.
            const storedHasPermissions1 = await contract.hasPermissions(
              caller.address,
              projectId1,
              operator1.address,
              [i]
            );
            const storedHasPermissions2 = await contract.hasPermissions(
              caller.address,
              projectId2,
              operator2.address,
              [i]
            );
            // Expect the permissions state to match.
            expect(storedHasPermissions1).to.be.equal(
              uniquePermissionIndexes1.includes(i)
            );
            expect(storedHasPermissions2).to.equal(false);
          })
        );
      });
    });
  });
  describe("Failure cases", () => {
    describe("removePermissionsFromOperators(...)", () => {
      it("Should revert if params dont match up", async () => {
        await expect(
          contract
            .connect(caller)
            .removePermissionsFromOperators(
              caller.address,
              [projectId1],
              [operator1.address, operator2.address],
              [permissionIndexes1, permissionIndexes2]
            )
        ).to.be.reverted;
        await expect(
          contract
            .connect(caller)
            .removePermissionsFromOperators(
              caller.address,
              [projectId1, projectId2],
              [operator1.address, operator2.address],
              [permissionIndexes1]
            )
        ).to.be.reverted;
      });
      it("Should revert if caller isnt msg.sender, nor the operator being removed", async () => {
        await expect(
          contract
            .connect(caller)
            .removePermissionsFromOperators(
              operator2.address,
              [projectId1],
              [caller, operator2.address],
              [permissionIndexes1, permissionIndexes2]
            )
        ).to.be.reverted;
      });
    });
  });
};
