...
import { transformPayEventRes } from '../utils/transformEventsData'
import PayEventElem from './PayEventElem'

/**
 * Returns a formatted element for any v4 project event
...
export function AnyProjectEvent({
  ...
  event: ProjectEventsQuery['projectEvents'][number] // import needs to change to v4
  tokenSymbol?: string
  ...
  // }
  if (event.payEvent) {
    const payEvent = transformPayEventRes(event.payEvent)
    return (
      <PayEventElem event={payEvent} withProjectLink={withProjectLink} />
    )
  }
