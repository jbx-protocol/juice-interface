...
import { MAX_DISTRIBUTION_LIMIT } from '../v2/math'
import {
...
export const hasDistributionLimit = (
  fundAccessConstraint: SerializedV3FundAccessConstraint | undefined,
): boolean => {
...
export const hasFundingDuration = (
  fundingCycle: Pick<SerializedV3FundingCycleData, 'duration'>,
...
const parameters: {
  name: keyof V3FundingCycleMetadata
  bits: 0 | 1 | 8 | 16
  parser?: (
    val: BigNumber,
  ) =>
...
}[] = [
  {
    ...
    parser: (val: BigNumber) =>
      val.toNumber() as V3FundingCycleMetadata['version'],
  },
  {
    name: 'global',
    bits: 16,
...
export const decodeV3FundingCycleMetadata = (
 ...
 */
export const getUnsafeV3FundingCycleProperties = (
  fundingCycle: V3FundingCycle,
): FundingCycleRiskFlags => {
  const metadata = decodeV3FundingCycleMetadata(fundingCycle.metadata)
 ...
 */
export const V3FundingCycleRiskCount = (
  fundingCycle: V3FundingCycle,
): number => {
  return Object.values(getUnsafeV3FundingCycleProperties(fundingCycle)).filter(
    v => v === true,
  ).length
}
