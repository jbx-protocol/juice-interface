...
export function JuiceListbox<T>(props: JuiceListboxProps<T>) {
  const { value, onChange, options, buttonClassName, className } = props
  return (
    <Listbox value={value} onChange={onChange}>
      <div className={twMerge('relative', className)}>
        <Listbox.Button
          className={twMerge(
            'flex w-full items-center justify-between gap-1 rounded-lg border border-smoke-300 bg-smoke-50 py-2 px-3 text-black dark:border-slate-300 dark:bg-slate-600 dark:text-slate-100 dark:placeholder:text-slate-300',
            buttonClassName,
          )}
        >
