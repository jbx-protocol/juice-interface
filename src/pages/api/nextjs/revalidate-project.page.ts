...
export default async function handler(
  ...
  // Update Supabase projects whenever a project needs revalidating. However, Supabase will only get updated if the new project data is already available in the subgraph, which can sometimes take a couple minutes
  await axios
    .get('/api/projects/update')
    // can throw error when env isnt set correctly
    .catch(err => console.error('Supabase projects update failed', err))
