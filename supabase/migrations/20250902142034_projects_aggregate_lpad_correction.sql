...
(
  ...
  join (
    select 
      ...
      lpad(coalesce(sum((current_balance)::numeric), 0)::text, 32, '0')::bpchar as current_balance,
      ...
      lpad(coalesce(sum(redeem_volume::numeric), 0)::text, 32, '0')::bpchar as redeem_volume,
      lpad(coalesce(sum(redeem_volume_usd::numeric), 0)::text, 32, '0')::bpchar as redeem_volume_usd,
      coalesce(sum(trending_payments_count), 0) as trending_payments_count,
      lpad(coalesce(sum(trending_score::numeric), 0)::text, 32, '0')::bpchar as trending_score,
      lpad(coalesce(sum(trending_volume::numeric), 0)::text, 32, '0')::bpchar as trending_volume,
      lpad(coalesce(sum(volume::numeric), 0)::text, 32, '0')::bpchar as volume,
      lpad(coalesce(sum(volume_usd::numeric), 0)::text, 32, '0')::bpchar as volume_usd
    from projects
