...
export async function queryAllSepanaProjects() {
  return axios.post<SepanaQueryResponse<SepanaProject>>(
...
export async function searchSepanaProjects(query = '', pageSize?: number) {
  return axios
    .post<SepanaQueryResponse<Json<SepanaProject>>>(
      `${process.env.NEXT_PUBLIC_SEPANA_API_URL}/search`,
      {
        engine_ids: [process.env.SEPANA_ENGINE_ID],
        query: {
 ...
 */
export async function writeSepanaDocs(docs: Json<SepanaProject>[]) {
  const jobs: string[] = []
  const projects: Json<SepanaProject>[] = []
