...
const loadJBProjectHandles = async (network: NetworkName) => {
  const contractJson = {
    abi: (
      await import(
...
const loadPublicResolver = (network: NetworkName) => {
...
const loadJBV1TokenPaymentTerminal = async (network: NetworkName) => {
  const contractJson = {
    abi: (
      await import(
...
const loadContract = async (
  ...
  if (contractName === V2ContractName.JBProjectHandles) {
    contractJson = await loadJBProjectHandles(network)
  } else if (contractName === V2ContractName.PublicResolver) {
    contractJson = loadPublicResolver(network)
  } else if (contractName === V2ContractName.JBV1TokenPaymentTerminal) {
    contractJson = await loadJBV1TokenPaymentTerminal(network)
  } else {
